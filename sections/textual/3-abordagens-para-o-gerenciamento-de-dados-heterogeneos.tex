\section{Abordagens para o Gerenciamento de Dados Heterogêneos}

Como estabelecido no capítulo anterior, este capítulo revisa as principais abordagens para o gerenciamento de dados heterogêneos. Isso porque, hodiernamente, grande parte das aplicações lidam com uma variedade significativa de tipos e formatos de dados. Apesar de muito frequente atualmente, este problema não é novo, portanto é importante compreender como as arquiteturas de persistência de dados evoluíram para atender a essa demanda crescente.

\subsection{O fim dos SGBDs generalistas e a ascensão da persistência poliglota}
Inicialmente, os SGBDs (principalmente os relacionais) eram usados de forma generalista, centralizando todos os dados, independentemente do formato, em um único banco de dados relacional. Essa abordagem, apesar de extremamente popular entre os anos 80 e 2000, se mostrou ineficiente para lidar com a variedade de dados, como discutido por \textcite{StonebrakerAndCetintemel2005}, que criticaram a ideia dominante chamada por eles de \textit{``one size fits all''} (``um tamanho serve para todos'', tradução livre) como algo que já não fazia mais sentido. Visto que, a tentativa de acomodar todos os tipos de dados em um único modelo reduzia drasticamente a eficiência antes oferecida pelos SGBDs generalistas, tornando-os incapazes de competir com abordagens especializadas. Neste mesmo artigo, os autores argumentam que a solução para o problema da variedade de dados seria o uso de múltiplos SGBDs especializados, cada um lidando com um tipo específico de dado, gerando um ganho de eficiência e performance que compensaria a complexidade adicional.

Essa solução, posteriormente nomeada como persistência poliglota \cite{Leberknight2011}, é amplamente adotada atualmente, com diversas soluções especializadas para diferentes tipos de dados, continuando a usar SGBD-Rs para dados estruturados, mas integrando bancos de dados NoSQL para dados não estruturados, bancos de dados em grafos para dados relacionais complexos, entre outros.
\begin{figure}[H]
    \centering
    \caption{Exemplo de arquitetura de persistência poliglota}
    \includegraphics[width=0.75\linewidth]{images/Fowler2011-polyglot-persistence.png}\\
    \fonteimagem{Fonte: Adaptado de \textcite{Fowler2011}}
...
\begin{itemize}
    \item \textbf{Coordenado pela aplicação:} A lógica da aplicação é responsável por interagir com múltiplos SGBDs, decidindo qual banco de dados usar para cada operação.
    \item \textbf{Orientado a serviços: } Cada serviço dentro de uma arquitetura de microsserviços é responsável por gerenciar seu próprio SGBD, comunicando-se com outros serviços conforme necessário.
    \item \textbf{Como um serviço:} Existe uma camada intermediária (\textit{middleware}) que serve de fachada para o acesso aos múltiplos bancos de dados.
...
Os dois primeiros tipos ("coordenado pela aplicação" e "orientado a serviços")representam o que a indústria moderna chama de persistência poliglota em sua forma mais comum. Essa prática de usar múltiplos SGBDs especializados em conjunto cria um desafio de engenharia que é um tema clássico na ciência da computação: a gestão de sistemas heterogêneos de bancos de dados \cite{Cardenas1987, Bright1992}.

\subsubsection{Desafios dos sistemas heterogêneos de bancos de dados}
A literatura sobre sistemas heterogêneos, que remonta há décadas, já identificava os desafios centrais dessa arquitetura \cite{Bright1992}. \textcite{Fowler2011}, ao prever o crescimento da tendência da persistência poliglota, e \textcite{Lajam2022}, ao analisar suas implementações práticas, destacam os mesmos desafios fundamentais:
\begin{itemize}
...
O conceito de bancos de dados multi-modelo é abordado e resumido por \textcite{PluciennikAndZgorzatek2017}, que, apesar de constatarem que o conceito não ter uma definição formal bem estabelecida, definem esse conceito como a capacidade de um único SGBD (que, em última instância, é baseado também em um único modelo) de suportar múltiplos formatos de dados (como relacional, documentos, grafos, etc.) em uma única plataforma. Permitindo, assim, que diferentes tipos de dados sejam armazenados e gerenciados no modelo apropriado para sua estrutura e seu processamento.

Dessa forma, os bancos de dados multi-modelo buscam oferecer a flexibilidade e a eficiência dos sistemas heterogêneos, mas com a simplicidade de gerenciar um único sistema, mitigando alguns dos desafios associados às abordagens tradicionais de persistência poliglota. Dessa forma, a abordagem multi-modelo emerge como uma tentativa de equilibrar a simplicidade de gestão dos antigos sistemas generalistas com a flexibilidade dos sistemas heterogêneos, buscando simplificar a complexidade de gerenciamento e a integração de dados inerente de um sistema heterogêneo de bancos de dados.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.75\linewidth]{images/PluciennikAndZgorzatek2017-multimodel-dbs-comparison.png}\\
...
\begin{itemize}
    \item \textbf{Métricas comuns:} As métricas de desempenho mais utilizadas nos estudos são Latência e Throughput. Métricas de consumo de recursos como uso de CPU e memória também são comuns, mas menos exploradas.
...
Para isso, os autores definiram um conjunto de dados, métricas e cargas de trabalho que simulam uma aplicação realista, com operações que incluem os diversos tipos de dados armazenados, assim fornecendo uma base consistente e reprodutível para comparar o desempenho de diferentes SGBDs.