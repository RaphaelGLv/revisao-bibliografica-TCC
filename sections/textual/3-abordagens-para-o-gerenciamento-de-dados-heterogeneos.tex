...
O desafio de gerenciar a heterogeneidade de dados -- um problema fundamental em aplicações contemporâneas como redes sociais -- tem impulsionado a evolução das arquiteturas de persistência. A necessidade de armazenar e processar eficientemente formatos distintos, como textos, imagens e relacionamentos complexos entre entidades, levou ao desenvolvimento de múltiplas estratégias, que serão exploradas neste capítulo.

Este capítulo, portanto, revisa a literatura sobre as principais arquiteturas de persistência de dados, com o objetivo de contextualizar o problema e fundamentar a análise comparativa de desempenho no caso de uso específico de uma rede social de nicho voltado para um contexto cristão de orações, respostas e interações entre os usuários, que será o foco deste trabalho.

\subsection{O fim dos SGBDs generalistas e a ascensão da persistência poliglota}
Inicialmente, os SGBDs (principalmente os relacionais) eram usados de forma generalista, centralizando todos os dados, independentemente do formato, em um único banco de dados. Essa abordagem, apesar de extremamente popular entre os anos 80 e 2000, se mostrou ineficiente para lidar com a variedade de dados, como discutido por \textcite{StonebrakerAndCetintemel2005}, que criticaram a ideia dominante chamada por eles de \textit{``one size fits all''} (``um tamanho serve para todos'', tradução livre) como algo que já não fazia mais sentido. Neste mesmo artigo, os autores argumentam que a solução para o problema da variedade de dados seria o uso de múltiplos SGBDs especializados, cada um lidando com um tipo específico de dado.
...
Diante do exposto, fica evidente que as redes sociais, com sua inerente heterogeneidade de dados, representam um caso de uso ideal para a aplicação e comparação das arquiteturas de sistemas heterogêneos de bancos de dados e de bancos de dados multimodelo. A revisão da literatura indica que, embora ambas as abordagens sejam viáveis, a escolha entre elas envolve um \textit{trade-off} fundamental entre a performance de sistemas especializados e a simplicidade de gestão de sistemas integrados.

Portanto, este trabalho se propõe a analisar empiricamente essas duas abordagens, utilizando como caso de uso uma rede social desenvolvida do zero e aplicada uma metodologia de benchmark, inspirada nos princípios do UniBench, possibilitando que informações cruciais sobre o desempenho, a escalabilidade e a complexidade de implementação sejam obtidas, contribuindo assim para a compreensão das melhores práticas na gestão de dados heterogêneos em aplicações modernas.